...
const createSchedule = async (userEmail, weeklySchedule, startDate) =>{
  ...
  try {
    ...
    let i= 0;
    while(i< 5){  ///   THIS CREATES 4 ORDERS NOW, SHOULD BE SET TO 22 LATER
...
const getSchedule = async(userEmail) =>{
  const orderArr = await orderService.getUpcomingOrdersByUser(userEmail);
  return orderArr
...
const deleteSchedule = async(userEmail) =>{
  ...
  const deletedItems = await orderService.deleteByEmail(userEmail);
